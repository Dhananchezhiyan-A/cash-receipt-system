...
const inputClass = 'px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-brand-500';
...
export default function ReceiptList() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-semibold text-slate-800">Transaction History</h1><p className="text-sm text-slate-500">{can('receipt.viewAll') ? 'All cash receipts and payment vouchers' : 'Your cash receipts and payment vouchers'}</p></div>

      <div className="bg-white border border-slate-200 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-3">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        ...
        <div className="px-4 py-3 border-t flex flex-wrap justify-between items-center gap-3 text-sm"><span className="text-slate-500">{meta.total} transactions | Page {query.page} of {meta.pages}</span><div className="flex gap-2 items-center"><select value={query.limit} onChange={(e) => setQuery({ limit: e.target.value })} className="border rounded px-2 py-1"><option value="10">10 per page</option><option value="25">25 per page</option><option value="50">50 per page</option></select><button disabled={query.page <= 1} onClick={() => setQuery({ page: query.page - 1 }, false)} className="p-1.5 border rounded disabled:opacity-40"><ChevronLeft size={16} /></button><button disabled={query.page >= meta.pages} onClick={() => setQuery({ page: query.page + 1 }, false)} className="p-1.5 border rounded disabled:opacity-40"><ChevronRight size={16} /></button></div></div>
      </div>
...
function PreviewDialog({ row, onClose }) {
  ...
  return <div className="fixed inset-0 z-50 bg-slate-900/50 p-4 overflow-auto"><div className="mx-auto max-w-5xl"><div className="no-print bg-white rounded-t-xl p-3 flex justify-end gap-2"><button onClick={() => window.print()} className={dialogButton}><Printer size={16} /> Print</button><button onClick={download} className={dialogButton}><Download size={16} /> PDF</button><button onClick={onClose} className="p-2"><X size={18} /></button></div><div ref={ref}><ReceiptTemplate type={type} data={data} /></div></div></div>;
}
...
function EditDialog({ row, onClose, onSaved }) {
  ...
  return <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4"><form onSubmit={submit} className="bg-white rounded-xl w-full max-w-xl p-5 space-y-3"><div className="flex justify-between"><h2 className="font-semibold">Edit {receipt ? 'Receipt' : 'Voucher'}</h2><button type="button" onClick={onClose}><X size={18} /></button></div>{[['number', 'Number'], ['date', 'Date'], ['party', receipt ? 'Received From' : 'Paid To'], ['purpose', 'Purpose'], ['amount', 'Amount'], ['signedBy', receipt ? 'Received By' : 'Approved By']].map(([key, label]) => <label key={key} className="block text-sm"><span className="block mb-1 font-medium">{label}</span><input required type={key === 'date' ? 'date' : key === 'amount' ? 'number' : 'text'} min={key === 'amount' ? '0.01' : undefined} step={key === 'amount' ? '0.01' : undefined} value={form[key]} onChange={(e) => update(key, e.target.value)} className={`${inputClass} w-full`} /></label>)}<label className="block text-sm"><span className="block mb-1 font-medium">Payment Mode</span><select value={form.paymentMode} onChange={(e) => update('paymentMode', e.target.value)} className={`${inputClass} w-full`}>{MODES.slice(1).map((mode) => <option key={mode}>{mode}</option>)}</select></label><div className="flex justify-end gap-2 pt-2"><button type="button" onClick={onClose} className="px-4 py-2 border rounded-lg text-sm">Cancel</button><button className="px-4 py-2 bg-brand-600 text-white rounded-lg text-sm">Save Changes</button></div></form></div>;
}
...
const dialogButton = 'inline-flex items-center gap-2 px-3 py-2 bg-slate-100 rounded-lg text-sm';
const Action = ({ title, onClick, danger, children }) => <button title={title} onClick={onClick} className={`p-2 rounded ${danger ? 'text-red-600 hover:bg-red-50' : 'text-slate-600 hover:bg-slate-100'}`}>{children}</button>;
